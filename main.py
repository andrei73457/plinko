...
from shapes import create_ball, create_peg, create_segment
...
y1, y2, y3, y4, y5 = b, HEIGHT // 4 - d, HEIGHT // 4, HEIGHT // 2 - 3 * b, HEIGHT - 4 * b
...
# i = 1, three pegs at 10, 11, 12
# i = 2, four pegs at 9, 10, 11, 12
# 12, 13, 13, 14, 14
...
for i in range(10):
    ...
    for j in range(WIDTH // step + 2):
        ...
        if j > (WIDTH // step + 2)//2 - i//2 - 2 and j < (WIDTH // step + 2)//2 + 2 + (i +1)//2:
            create_peg(peg_x, peg_y, space, 'gray')
...
while True:
    ...
    if dt > 1:
        dt = 0
        balls.append(((230, 0,0), create_ball(space)))
